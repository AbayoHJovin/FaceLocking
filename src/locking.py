...
class FaceLockingSystem:
    def __init__(self, db_path: Path, model_path: str = "models/arcface.onnx"):
        ...
        self.move_left_timer = 0.0
        self.move_right_timer = 0.0
        self.move_display_duration = 0.8 # Seconds to show label
    ...
    def process_frame(self, frame: np.ndarray):
        ...
        if target_f:
            ...
            if self.prev_centroid is not None:
                ...
                now = time.time()
                if dx > 20: 
                    self.record_action("MOVE", "Moved Right")
                    self.move_right_timer = now + self.move_display_duration
                    self.move_left_timer = 0 # Mutually exclusive
                elif dx < -20: 
                    self.record_action("MOVE", "Moved Left")
                    self.move_left_timer = now + self.move_display_duration
...
def main():
    ...
    while True:
        ...
        if system.is_locked:
            ...
            now = time.time()
            if now < system.move_left_timer:
                cv2.putText(vis, "MOVING LEFT", (10, y_off), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
                y_off += 25
            elif now < system.move_right_timer:
                cv2.putText(vis, "MOVING RIGHT", (10, y_off), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
                y_off += 25
